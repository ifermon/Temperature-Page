<?php
	...
	$mysqli = new mysqli("localhost", "weather_program", "", "weather");
	//$mysqli = new mysqli();
	...
	function getLastTemps($mysqli) {
		$stmt = $mysqli->prepare("SELECT temperature as temp1, ts as created_at FROM minute_data ORDER BY ts DESC LIMIT 1");
		$stmt->execute();
	...
	function getAVGTemps($mysqli) {
		$stmt = $mysqli->prepare("SELECT AVG(temperature) as temp1 FROM minute_data WHERE ts >= SYSDATE() - INTERVAL 1 DAY");
		$stmt->execute();
	...
	function last24hourValues($mysqli) {
		$stmt = $mysqli->prepare("SELECT temperature as temp1, ts as created_at FROM minute_data WHERE ts >= SYSDATE() - INTERVAL 1 DAY");
		$stmt->execute();
		$stmt->bind_result($res['temp1'], $res['created_at']);
		$rows = array();
	...
	<body>
		<div class="content">
			<div class="thermometers">
				<div class="label">Outdoor Sensor</div>
				 <div class="de">
					<div class="den">
					  <div class="dene">
						<div class="denem">
                ...
                <div class="details">24h Average<br /><?php print number_format($lastAVGTemps['temp1'], 1); ?>&deg;</div>
